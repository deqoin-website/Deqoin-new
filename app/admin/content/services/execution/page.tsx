...
const DEFAULT_EXECUTION_ITEMS: CategoryItem[] = uygulamaBirimleri.map(({ slug, title, sideLabel, image }) => ({
  href: `/admin/content/services/execution/${slug}`,
  title,
  sideLabel,
  image,
  slug,
...
const TAB_ITEMS: Array<{ key: TabKey; label: string; description: string; icon: typeof FileText }> = [
  ...
  { key: 'birimler', label: 'Birimler', description: 'Uygulama kartlarını yönet', icon: FolderKanban },
];
  ...
  sections: [
    {
      ...
      blur: 0,
      overlay: 30,
      slides: [SLIDER_IMAGE_URLS.execution],
    },
    {
      id: 'cta',
      type: 'cta',
...
const normalizeContent = (value: any): PageContent => {
  ...
  const mappedItems = Array.isArray(categories?.items) && categories.items.length > 0
    ? categories.items.map((item: any) => ({
        href: item?.href || `/admin/content/services/execution/${item?.slug || 'yeni-birim'}`,
        title: item?.title || 'Yeni Birim',
        sideLabel: item?.sideLabel || 'Execution Detail',
        image: item?.image || SLIDER_IMAGE_URLS.execution,
        slug: item?.slug || 'yeni-birim',
      }))
    : DEFAULT_EXECUTION_ITEMS;

  return {
    page: value?.page || 'execution',
...
export default function ExecutionEditor() {
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      ...
      console.error('Execution content load error:', error);
      ...
      showToast('Uygulama birimi içerik yüklenemedi.', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [showToast]);
  ...
  const saveContent = async (nextContent = content) => {
    ...
    try {
      ...
      showToast('Uygulama birimi içerik kaydedildi.', 'success');
    } catch (error) {
      console.error('Execution content save error:', error);
  ...
  const categoryItems = Array.isArray(categorySection?.items) ? categorySection.items : DEFAULT_EXECUTION_ITEMS;
  ...
  const addItem = () => {
    mutateContent((draft) => {
      ...
      categories?.items?.push({
        href: '/admin/content/services/execution/yeni-birim',
        title: 'Yeni Birim',
        sideLabel: 'Execution Detail',
        image: SLIDER_IMAGE_URLS.execution,
        slug: 'yeni-birim',
      });
    });
  ...
  const removeItem = async (index: number) => {
    const ok = await premiumConfirm({
      title: 'BİRİMİ SİL',
      message: 'Bu uygulama birimini silmek istediğinize emin misiniz?',
      confirmText: 'SİL',
      cancelText: 'VAZGEÇ',
      isDanger: true,
    });
  ...
  const openDetail = (item: CategoryItem) => `/admin/content/services/execution/${item.slug}`;

  const apiCards = [
    {
      title: 'Sayfa içeriği',
      href: '/api/content?page=execution',
      status: apiStatus.content,
      note: 'Hero, CTA ve uygulama birimleri bu kayıt üzerinden okunup yazılır.',
    },
    {
      title: 'Upload servisi',
      href: '/api/upload',
      status: apiStatus.upload,
      note: 'Kapak, slider ve birim görselleri için kullanılır.',
    },
  ];
        ...
        <div className="flex flex-col gap-6 p-5 sm:p-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl space-y-4">
            <Badge className="border border-[color:var(--line)] bg-[color:var(--surface-muted)] text-[color:var(--text-muted)]">
              ...
              UYGULAMA BİRİMİ GENEL AYARLARI
            </Badge>
            <div className="space-y-2">
              <h1 className="text-3xl font-semibold tracking-tight text-[color:var(--text)] sm:text-4xl">
                Uygulama birimi girişini yönetin
              </h1>
              <p className="max-w-2xl text-sm leading-7 text-[color:var(--text-muted)]">
                Hero, CTA ve uygulama birimi kartları aynı ekranda düzenlenir. Görsel yüklemeleri
                test edilir ve ekran light/dark tema ile uyumlu çalışır.
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[470px]">
            ...
            <div className="rounded-2xl border border-[color:var(--line)] bg-[color:var(--surface-muted)] p-4">
              <p className="text-[0.65rem] uppercase tracking-[0.28em] text-[color:var(--text-muted)]">Birim</p>
          ...
          <CardContent className="space-y-6 p-5 sm:p-6">
            ...
            {activeTab === 'birimler' && (
              <div className="space-y-4">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                  <div>
                    <h3 className="text-base font-semibold text-[color:var(--text)]">Uygulama Birimleri</h3>
                    <p className="text-sm text-[color:var(--text-muted)]">
                      İnşaat ve uygulama kartlarını düzenleyin, detay sayfalarına gidin.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button
                    ...
                    <Button asChild className="bg-[color:var(--accent)] text-[color:var(--text-inverse)] hover:bg-[color:var(--accent-soft)]">
                      <Link href="/admin/studios/insaat-ekipleri">
                  ...
                  {categoryItems.map((item, index) => (
                    <Card key={`${item.slug}-${index}`} className="overflow-hidden border border-[color:var(--line)] bg-[color:var(--surface-muted)] shadow-[0_20px_60px_rgba(0,0,0,0.12)]">
                      <AdminImageDropzone
                        ...
                        description="Birim kartı görselini sürükleyip bırakın."
                        emptySubtitle="Kart görselini sürükleyin ya da tıklayıp seçin."
                        emptyTitle="Birim görseli"
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              mutateContent((draft) => {
                                ...
                                categories.items[index].href = `/admin/content/services/execution/${event.target.value}`;
                              })
                            }
                            className="h-11 rounded-2xl border-[color:var(--line)] bg-[color:var(--surface)] text-[color:var(--text)]"
                          />
        ...
        <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
          <Card className="border border-[color:var(--line)] bg-[color:var(--surface)] shadow-[var(--shadow)]">
            <CardHeader className="border-b border-[color:var(--line)]">
              ...
              <CardDescription className="text-[color:var(--text-muted)]">
                Uygulama birimi içerik ve upload uçları.
              </CardDescription>
            </CardHeader>
          ...
          <Card className="border border-[color:var(--line)] bg-[color:var(--surface)] shadow-[var(--shadow)]">
            <CardHeader className="border-b border-[color:var(--line)]">
              <CardTitle className="text-lg text-[color:var(--text)]">Yeni Birim</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 p-5 sm:p-6">
              <Button
                type="button"
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-[color:var(--line)] bg-[color:var(--surface-muted)] px-5 py-4 sm:px-6">
                <div>
                  <p className="text-[0.65rem] uppercase tracking-[0.32em] text-[color:var(--text-muted)]">Yeni Birim</p>
                ...
                <div className="space-y-4 p-5 sm:p-6">
                  ...
                  />
                  <Input
                    value={newItem.sideLabel}
                    onChange={(event) => setNewItem((prev) => ({ ...prev, sideLabel: event.target.value }))}
                    className="h-12 rounded-2xl border-[color:var(--line)] bg-[color:var(--surface-muted)] text-[color:var(--text)]"
                  ...
                  <div className="sm:col-span-2">
                    <AdminImageDropzone
                      ...
                      accept="image/*"
                      buttonLabel="Görsel seç"
                      description="Yeni birim için görsel yükleyin."
                      emptySubtitle="Birim görselini sürükleyin veya tıklayıp seçin."
                      emptyTitle="Birim görseli"
                      previewAlt={newItem.title || 'Yeni birim'}
                      previewUrl={newItem.image}
                      title="Birim görseli"
                      onFileSelect={async (file) => {
                        const url = await uploadFile(file);
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    mutateContent((draft) => {
                      const categories = draft.sections.find((section) => section.id === 'categories');
                      categories?.items?.push({
                        href: `/admin/content/services/execution/${newItem.slug}`,
                        title: newItem.title,
                        sideLabel: newItem.sideLabel,
                        image: newItem.image || SLIDER_IMAGE_URLS.execution,
                        slug: newItem.slug,
                      });
                    });
                    setIsAddModalOpen(false);
                    setNewItem({ title: '', sideLabel: '', slug: '', image: SLIDER_IMAGE_URLS.execution });
                  }}
                >
                  Kartı Ekle
                </Button>
