...
const TAB_ITEMS: Array<{ key: TabKey; label: string; description: string; icon: typeof Wrench }> = [
  ...
  { key: 'surec', label: 'Süreç', description: 'Uygulama hikayesi ve adımlar', icon: Wrench },
  { key: 'odak', label: 'Odak', description: 'Öne çıkan uygulama noktaları', icon: Target },
...
const makeSeed = (slug: string): DepartmentState => {
  const matched = uygulamaBirimleri.find((item) => item.slug === slug);

  return {
    slug,
    title: matched?.title || slug.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase()),
    sideLabel: matched?.sideLabel || 'Execution Studio',
    description: matched?.description || 'Bu uygulama birimi için içerik henüz tanımlanmadı.',
    image: matched?.image || SLIDER_IMAGE_URLS.execution,
    mediaType: 'image',
    heroBlur: 0,
    heroOverlay: 30,
...
export default function ExecutionDetailEditor() {
  const params = useParams();
  const rawSlug = params?.slug;
  const slug = Array.isArray(rawSlug) ? rawSlug[0] : rawSlug || 'insaat-ekipleri';
  ...
  const loadDepartment = useCallback(async () => {
    ...
    try {
      ...
      showToast('Uygulama birimi yüklenemedi.', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [showToast, slug]);
  ...
  const saveDepartment = async (nextDepartment = department) => {
    ...
    try {
      ...
      showToast('Uygulama birimi kaydedildi.', 'success');
    } catch (error) {
          ...
          <div className="max-w-3xl space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm leading-7 text-[color:var(--text-muted)]">
                Bu birimin tüm yayın katmanlarını tek ekranda düzenleyin. Hero, süreç, odak ve
                  ...
                  <Card className="border border-[color:var(--line)] bg-[color:var(--surface-muted)] shadow-none">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <Button
                        ...
                        onClick={() => document.getElementById('execution-detail-slider-add')?.click()}
                      >
                        <Plus className="mr-2 h-4 w-4" />
                        Slider Görseli Ekle
                      </Button>
                      <input
                        id="execution-detail-slider-add"
                        type="file"
                        accept="image/*"
                        className="hidden"
                        onChange={async (event) => {
                      ...
                      <CardContent className="flex items-center justify-between gap-2 p-4">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() => document.getElementById(`execution-detail-slider-replace-${index}`)?.click()}
                          >
                          ...
                          <input
                            id={`execution-detail-slider-replace-${index}`}
                            type="file"
                            accept="image/*"
                            className="hidden"
                            onChange={async (event) => {
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Button
                    type="button"
                    className="bg-[color:var(--accent)] text-[color:var(--text-inverse)] hover:bg-[color:var(--accent-soft)]"
                    onClick={() => mutateDepartment((draft) => draft.process.push({ title: 'Yeni Adım', desc: 'Açıklama' }))}
                  >
                    <Plus className="mr-2 h-4 w-4" />
                    Adım Ekle
                  </Button>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-[color:var(--text-muted)]">Uygulama birimi etiketleri.</p>
                  </div>
                  <Button
                    type="button"
                    className="bg-[color:var(--accent)] text-[color:var(--text-inverse)] hover:bg-[color:var(--accent-soft)]"
        ...
        <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
          <Card className="border border-[color:var(--line)] bg-[color:var(--surface)] shadow-[var(--shadow)]">
            <CardHeader className="border-b border-[color:var(--line)]">
              ...
              <CardDescription className="text-[color:var(--text-muted)]">
                Uygulama birimi departman ve upload uçları.
              </CardDescription>
            </CardHeader>
          ...
          <Card className="border border-[color:var(--line)] bg-[color:var(--surface)] shadow-[var(--shadow)]">
            ...
            <CardContent className="space-y-3 p-5 sm:p-6">
              <Button asChild variant="outline" className="w-full border-[color:var(--line)] bg-[color:var(--surface-muted)] text-[color:var(--text)]">
                <Link href="/admin/content/services/execution">
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  Uygulama Birimi Genel Ayarlar
                </Link>
              </Button>
              <Button asChild variant="outline" className="w-full border-[color:var(--line)] bg-[color:var(--surface-muted)] text-[color:var(--text)]">
